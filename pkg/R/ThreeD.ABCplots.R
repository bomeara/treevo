...
param.position<-parameter
	...
	if (max(subset(a[which(a$weight>0),])[,param.position])-min(subset(a[which(a$weight>0),])[,param.position])!=0) {
	v<-vector("list", max(a$generation))
		...
		title3d(colnames(a)[param.position], col='red', pos=c(NA, -2, max(z))) 
...
short.kept<-subset(kept[which(kept$generation>1),])[,]
	...
	if (show.particles=="none"){
		...
		#	spheres3d(x=reject[i, param.position], y=-1*(.05*(max(y)-min(y))), z=reject[i, 1], radius=.3, type="s", col="gray", add=TRUE, aspect=TRUE)
		...
		for (j in 1:nrow(kept)) {
			...
			#rgl.points("points", x=kept[j, param.position], y=-1*(.05*(max(y)-min(y))), z=kept[j, 1], radius=circle.size, type="s", col="black", add=TRUE, aspect=TRUE)
