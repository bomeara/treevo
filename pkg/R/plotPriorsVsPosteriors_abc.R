...
if(class(data)=="data.frame" || class(data)=="matrix"){
			data1<-cbind(rep(0, length(data[,1])), rep(0, length(data[,1])), rep(0, length(data[,1])), rep(0, length(data[,1])), rep(0, length(data[,1])), rep(1, length(data[,1])), data)
	...
	all<-as.data.frame(cbind(run, data1))
}
	data1<-c()
if(class(data)=="list"){
	all<-data.frame()
	for (list in 1:length(data)) {
		data1<-cbind(rep(0, length(data[[list]][,1])), rep(0, length(data[[list]][,1])), rep(0, length(data[[list]][,1])), rep(0, length(data[[list]][,1])), rep(0, length(data[[list]][,1])), rep(1, length(data[[list]][,1])), data[[list]])
...
names(all)<-c("run", "generation", "attempt", "id", "parentid", "distance", "weight", paste("param", 1:((dim(all)[2])-7), sep=""))
