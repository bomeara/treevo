boxcoxEstimation<-function (summaryValuesMatrix) {
  ...
  for (summaryValueIndex in 1:dim(summaryValuesMatrix)[2]) {
    ...
    lowValue <- min(summaryValuesMatrix[, summaryValueIndex]) - 
      4 * sd(summaryValuesMatrix[, summaryValueIndex])
    if (lowValue <= 0) {
    ...
    if (sd(summaryValuesMatrix[, summaryValueIndex]) > 0) {
      newLambda <- as.numeric(try(powerTransform(summary, 
                                                 method = "Nelder-Mead")$lambda))
      if (!is.na(newLambda)) {
