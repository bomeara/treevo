...
#' \code{nearestNeighborDisplacementExtrinsic} with parameters \code{params = sigma, springK, maximumForce}
#' 
...
#' The input parameters for this model are:
#' \code{everyoneDisplacementExtrinsic} with parameters \code{params = sigma, springK, maximumForce}
#' 
...
#' The input parameters for this model are:
#' \code{ExponentiallyDecayingPushExtrinsic} with parameters \code{params = sigma, maximumForce, halfDistance}
#' 
...
#' @rdname extrinsicModels
#' @export
nearestNeighborDisplacementExtrinsic <- function(
		params, selfstates, otherstates, timefrompresent) {
	#
    #params[1] is sigma, params[2] is springK, params[3] is maxforce
    ...
    if(localsign == 0) {
        localsign = sign(rnormFastZig(nZig = 1))    
		}
    newdisplacement <- rnormFastZig(
		nZig = 1, 
		meanZig = localsign * min(
			c( abs(
...
everyoneDisplacementExtrinsic <- function(params, selfstates, otherstates, timefrompresent) {
    ...
    for (i in 1:length(otherstates)) {
		...
		netforce <- netforce+localsign*min(
			c(
				abs(springK/
					((selfstates[1]-otherstates[i])*(selfstates[1]-otherstates[i]))
					)
				, maxforce)
			, na.rm = TRUE)
		}
...
ExponentiallyDecayingPushExtrinsic <- function(params, selfstates, otherstates, timefrompresent) {
    #params[1] is sigma, params[2] is maxForce when character difference = 0, params[3] is half
    ...
    newdisplacement <- rnormFastZig(
		nZig = 1, 
		meanZig = maxForce*localsign*exp(-1*rate*abs((selfstates[1]-repulsorValue))), 
		sdZig = sigma)
    return(newdisplacement)
	}
...
GetExpPushPriors <- function(numSteps, phy, data) {
	...
	sigma <- getBMRatePrior(phy=phy, traits=data, timeStep=timeStep)  #new TreEvo function
	...
	max.push.prior <- EstimateMaxForce(
		data.max.diff, 
		halfDistance = qexp(0.5, half.distance.prior),
		numSteps)
	#each one is doubled so they will exp rate param will fit
		# into intrinsicPriorValues matrix in doRun_rej
	res <- matrix(
		c(sigma, sigma, 
			half.distance.prior, half.distance.prior, 
			max.push.prior, max.push.prior), 
		nrow = 2)
	return(res)  
	}
...
ForceOptimality <- function(x, halfDistance, numSteps, diff) {
	res <- abs(diff - MultiStepPush(x, halfDistance, numSteps))
	return(res)
	}
