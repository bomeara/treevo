...
initializeStatesFromMatrices <- function(particle, startingPriorsValue, startingPriorsFns, intrinsicPriorsValues, intrinsicPriorsFns, extrinsicPriorsValues, extrinsicPriorsFns) {
	particle$startingValues  <- rep(NA,length=dim(startingPriorsValue)[2])
	...
	for (j in sequence(dim(startingPriorsValue)[2])) {
		particle$startingValues[j] <- pullFromPrior(priorValues=startingPriorsValue[, j], priorFn=startingPriorsFns[j])
	}
...
mutateStates <- function(particle, startingPriorsValue, startingPriorsFns, intrinsicPriorsValues, intrinsicPriorsFns, extrinsicPriorsValues, extrinsicPriorsFns, standardDevFactor) {
	for (j in sequence(dim(startingPriorsValue)[2])) {
		particle$startingValues[j] <- mutateState(startingState=particle$startingValues[j], standardDevFactor=standardDevFactor, priorValues=startingPriorsValue[, j], priorFn=startingPriorsFns[j])
	}
	for (j in sequence(dim(intrinsicPriorsValues)[2])) {
