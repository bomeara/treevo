...
The function `highestPostDens` provides us with the  weighted mean, standard deviation, upper and lower highest posterior density (HPD) for each free parameter in the posterior sample, for each individual run.

For example, for each Brownian Motion run:

```{r}

highestPostDens(resultsBM[[1]]$particleDataFrame, percent=0.95, returnData=FALSE)
highestPostDens(resultsBM[[2]]$particleDataFrame, percent=0.95, returnData=FALSE)
highestPostDens(resultsBM[[3]]$particleDataFrame, percent=0.95, returnData=FALSE)
```

Or just to look at the first run of the Bounded BM run:

```{r}
highestPostDens(resultsBound[[1]]$particleDataFrame, percent=0.95, returnData=FALSE)
